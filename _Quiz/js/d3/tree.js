function tree(rootNode){
	var svgW=320, svgH =283, vRad=17, tree={cx:160, cy:20, w:40, h:40};
	tree.vis={v:0, l:rootNode, p:{x:tree.cx, y:tree.cy},c:[]};	
	tree.size=1;
	tree.glabels =[];
	tree.incMatx =[];
	...
	tree.addLeaf = function(_, name){
		function addLeaf(t){
			if(t.v==_){ t.c.push({v:tree.size++, l:name, p:{},c:[]}); return; }
			t.c.forEach(addLeaf);
		}
		addLeaf(tree.vis);
		reposition(tree.vis);
	...
	redraw = function(){
		...
		circles.enter().append('circle').attr('cx',function(d){ return d.f.p.x;}).attr('cy',function(d){ return d.f.p.y;})
			.attr('r',vRad).attr("node",function(d){ return d.l.split("-")[0];})
			.attr("shortpath",function(d){ return d.l.split("-")[2];})
			.attr("distance",function(d){ return d.l.split("-")[1];})
			//.on('click',circleClick)//return tree.addLeaf(d.v);
		...
		labels.text(function(d){
			return d.l.split("-")[0]+"("+d.l.split("-")[1]+")";
		}).transition().duration(500)
	...
	treeAddClassTocircle = function(_,className) {
		function f(t) {
			if(t.v==_){ 
				var txt_y = Number(t.p.y)+5;
				$("circle[cx="+ t.p.x +"][cy="+ t.p.y +"]").attr("class",className);
				$("text[x="+ t.p.x +"][y="+ txt_y +"]").attr("class",'visited-text');
				$("line[x2="+ t.p.x +"][y2="+ t.p.y +"]").attr("class",'visited-line'); 
				return; 
			}
			t.c.forEach(f);
		}
	...
	treeAddAttr = function(_,name,value){
		function f(t) {
			if(t.v==_){ 
				$("circle[cx="+ t.p.x +"][cy="+ t.p.y +"]").attr(name,value); 
				return; 
			}
			t.c.forEach(f);
		}
	...
	initialize = function(){
		...
		d3.select("#navdiv").append("svg").attr("width", svgW).attr("height", svgH).attr('id','treesvg');
		...
		d3.select("#treesvg").append('g').attr('id','g_circles').selectAll('circle').data(tree.getVertices()).enter()
			...
			.attr("class","visited").attr("node",function(d){ return d.l;})
			.attr("node",function(d){ return d.l.split("-")[0];})
			.attr("shortpath",function(d){ return d.l.split("-")[2];})
			.attr("distance",function(d){ return d.l.split("-")[1];})
			//.on('click',circleClick);//return tree.addLeaf(d.v);
			
		d3.select("#treesvg").append('g').attr('id','g_labels').selectAll('text').data(tree.getVertices()).enter().append('text')
			.attr('x',function(d){ return d.p.x;}).attr('y',function(d){ return d.p.y+5;}).text(function(d){return d.l.split("-")[0]+"("+d.l.split("-")[1]+")";})
			//.on('click',function(d){});
